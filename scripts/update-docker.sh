...
# Career Assistant — one-command Docker update.
#
# Updates an existing standalone Docker install: git pull (best-effort) →
# rebuild images (or pull when CAREER_IMAGE points at a registry) → up -d →
# wait for the app healthy.
#
# Usage:
...
# not an error, and docker/.env is never overwritten.
