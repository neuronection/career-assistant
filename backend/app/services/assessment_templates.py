"""Assessment template library service.
...
(unknown keys auto-propose on import — availability over curation;
promotion governs the vocabulary), and execution by compiling onto the
 engine (runs gain template_id; nothing new at runtime).
"""
...
def generate_template_ref() -> str:
    """8-char Crockford base32 share ref."""
    import secrets
...
class TemplateService:
        ...
        for phase in content.phases:
            for question in phase.questions:
                ...
                if question.dimension_key:
                    dimensions.add(question.dimension_key)
                for option in question.options:
                    keys.update((option.scores.skill_levels or {}).keys())
                    dimensions.update((option.scores.dimension_levels or {}).keys())
                for statement in question.statements:
                    keys.update((statement.scores.skill_levels or {}).keys())
                    dimensions.update((statement.scores.dimension_levels or {}).keys())
    ...
    async def _resolve_dimension_keys(self, keys: list[str]) -> dict:
        """Every referenced dimension must be a registry key.

        Unlike skills, dimensions are never auto-proposed — the registry
        is the closed metric language, so unknown keys hard-reject in
        both authoring and import.
        """
        if not keys:
            return {"unknown": [], "resolved": 0}
    ...
    async def create_template(
        ...
        *,
        ...
        if visibility == TemplateVisibility.PUBLIC.value:
            raise ValidationError("Public visibility is not reachable yet")
    ...
    async def start_run(self, user_id: UUID, template_id: UUID) -> AssessmentRun:
        """Compile the template onto the engine: one run with
    ...
    async def compile_results(self, run: AssessmentRun) -> dict:
        ...
        for answer in run.answers:
            ...
            for key, value in (derived.get("dimension_levels") or {}).items():
                raw_dimensions[key] = raw_dimensions.get(key, 0.0) + float(value)
        ...
        return {
            "raw_scores": raw,
            "levels": levels,
            "raw_dimension_scores": raw_dimensions,
