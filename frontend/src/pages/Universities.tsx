...
export function Universities() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6" data-testid="universities">
      <p className="text-slate-500">{t("universities.subtitle")}</p>
      ...
      <label className="block border-2 border-dashed border-slate-300 rounded-xl p-8 text-center cursor-pointer hover:border-primary-500 bg-white">
        ...
        <p className="mt-2 text-sm font-medium">
          {uploading ? t("universities.uploading") : t("universities.dropHint")}
        </p>
        <p className="text-xs text-slate-400">{t("universities.fileLimits")}</p>
        {uploading && parseJob && (
          <div className="mt-3 max-w-xs mx-auto" data-testid="parse-progress">
            <JobFlowStatus job={parseJob} title={t("universities.parsingCatalog")} />
          </div>
        )}
      </label>
      ...
      <section className="space-y-3">
        <h2 className="font-semibold">{t("universities.yourUniversities")}</h2>
        {universities.length === 0 ? (
          <EmptyState
            icon={Building2}
            compact
            title={t("universities.noneYetTitle")}
            description={t("universities.noneYetBody")}
          />
        ) : (
