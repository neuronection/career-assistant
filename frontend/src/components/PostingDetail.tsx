...
function LevelDots({ level }: { level: number }) {
  return (
    <span
      ...
      data-testid="skill-level-dots"
      aria-label={i18next.t("postingDetail.levelAria", { level })}
    >
...
function SkillChip({ skill }: { skill: PostingExtractSkill }) {
  ...
  const label = skill.unresolved
    ? t("postingDetail.unmapped", {
        label: skill.raw_label ?? t("postingDetail.unknown"),
      })
    : skill.skill_key ?? t("postingDetail.unknown");
  return (
    <span className="inline-flex items-center gap-1.5 border border-slate-200 rounded-lg px-2 py-1 text-xs">
      ...
      <span className={`px-1.5 py-0.5 rounded text-[10px] ${PRIORITY_STYLES[skill.priority]}`}>
        {t(`postingDetail.priority.${skill.priority}`, {
          defaultValue: skill.priority.replace(/_/g, " "),
        })}
      </span>
      <button
        type="button"
        aria-label={t("postingDetail.showEvidence")}
...
/**
 * Posting detail: renders the structured extract — skill chips
...
const GATE_LABEL_KEYS: Record<string, string> = {
  travel: "postingDetail.gate.travel",
  shift: "postingDetail.gate.shift",
  hours: "postingDetail.gate.hours",
  salary_min: "postingDetail.gate.salaryMin",
};
...
export function PostingDetail({
  ...
  async function practiceInterview() {
    ...
    try {
      ...
      setInterviewError(apiDetail(err));
  ...
  return (
    <Modal open onOpenChange={(o) => !o && onClose()}>
      <ModalContent size="lg" aria-describedby={undefined}>
        <ModalHeader>
          <ModalTitle>
            {posting?.title ?? t("common.loading")}
            {posting?.ref && (
              <span
                className="ml-2 text-xs font-mono text-slate-400 align-middle"
                title={t("postingDetail.refTitle")}
        ...
        {!posting ? (
          <p className="text-sm text-slate-400 p-4">{t("postingDetail.loadingPosting")}</p>
        ) : (
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            ...
            >
              {t(`postingCard.provenanceValue.${provenance}`, {
            ...
            {posting.url && (
              ...
              >
                <ExternalLink className="w-3 h-3" /> {t("postingDetail.originalPosting")}
              </a>
            )}
            <Button
              variant="outline"
            ...
            <Button
              ...
              onClick={() => void practiceInterview()}
              data-testid="practice-interview-posting"
            >
              <GraduationCap className="mr-1 h-3.5 w-3.5" />
              {t("postingDetail.practiceInterview")}
            </Button>
          </div>
          {interviewError && (
            <p
          ...
          <p className="text-slate-500">
            ...
            {posting.salary_min != null
              ? ` · ${posting.salary_currency ?? ""} ${Number(posting.salary_min).toLocaleString()}${
                ...
                } / ${posting.salary_period ?? t("postingDetail.year")}`
              : ""}
          </p>
          ...
          >
            <span>
              {t("postingDetail.sourceLabel")}{" "}
              <span className="text-slate-600">
                {posting.source_title || posting.source_key || posting.source_connector || t("postingDetail.unknown")}
              </span>
            </span>
            {posting.source_synced_at && (
              <span>
                {t("postingDetail.synced", {
                  date: new Date(posting.source_synced_at).toLocaleDateString(),
                })}
              </span>
            )}
            {posting.url && (
              <a
                ...
                className="inline-flex items-center gap-1 text-primary-700 hover:underline"
              >
                <ExternalLink className="w-3 h-3" /> {t("postingDetail.originalListing")}
              </a>
            )}
          </div>
          ...
          {match && (
            <section className="border border-slate-100 rounded-xl p-3" data-testid="match-card">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-slate-400 uppercase">{t("postingDetail.matchScore")}</h3>
              ...
              {match.estimate && (
                <p className="text-[11px] text-amber-600 mt-1" data-testid="match-estimate-note">
                  {t("postingDetail.estimateNote")}
                </p>
              )}
              {match.gates && match.gates.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5" data-testid="posting-gates">
                  {match.gates.map((gate) => (
                    <span
                      key={gate}
                      className="rounded bg-amber-50 px-2 py-0.5 text-[11px] text-amber-700"
                      title={t("postingDetail.gateTitle")}
                    >
                      {GATE_LABEL_KEYS[gate] ? t(GATE_LABEL_KEYS[gate]) : gate}
                    </span>
                  ))}
                </div>
              )}
                ...
                {Object.entries(match.breakdown).map(([dim, entry]) => (
                  <div key={dim} className="flex items-center gap-2 text-xs">
                    <span className="w-28 shrink-0 text-slate-500">
                      {t(`dimensions.${dim}`, { defaultValue: dim.replace(/_/g, " ") })}
                    </span>
          ...
          {extract?.responsibilities && extract.responsibilities.length > 0 && (
            <section>
              <h3 className="text-xs font-semibold text-slate-400 uppercase mb-1.5">
                {t("postingDetail.responsibilitiesHeading")}
              </h3>
                  ...
                  <li key={i} className="flex items-center gap-2 text-slate-600">
                    ...
                    {r.time_pct != null && (
                      <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">
                        {t("postingDetail.timePct", { percent: r.time_pct })}
                      </span>
                    )}
                  </li>
                ))}
          ...
          {(extract?.benefits?.length ?? 0) > 0 && (
            <section>
              <h3 className="text-xs font-semibold text-slate-400 uppercase mb-1.5">
                {t("postingDetail.benefitsHeading")}
              </h3>
          ...
          {(posting.similar?.length ?? 0) > 0 && (
            <section>
              <h3 className="text-xs font-semibold text-slate-400 uppercase mb-1.5">
                {t("postingDetail.similarHeading")}
              </h3>
