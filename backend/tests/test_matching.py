...
async def _job_id(client, auth_headers, code: str) -> str:
    job = (await client.get(f"/api/v1/jobs/{code}", headers=auth_headers)).json()
    return job["id"]


async def test_compare_returns_rows_in_request_order(
    client, auth_headers, profile_ready, seeded_catalog
):
    dev = await _job_id(client, auth_headers, "software-developer")
    nurse = await _job_id(client, auth_headers, "nurse")
    physician = await _job_id(client, auth_headers, "physician")
    response = await client.get(
        f"/api/v1/match/compare?job_ids={dev},{nurse},{physician}",
        headers=auth_headers,
    )
    assert response.status_code == 200, response.text
    rows = response.json()
    assert [r["job"]["code"] for r in rows] == [
        "software-developer",
        "nurse",
        "physician",
    ]
    for row in rows:
        ...
        assert row["breakdown"]["dimensions"]["skills"]["score"] is not None
    physician_row = rows[2]
    assert physician_row["gated"] is True
...
async def test_compare_matches_rankings_scores(
    ...
    for item in rankings["items"]:
        code = item["job"]["code"]
        if code in compared:
            assert compared[code]["fit_score"] == item["fit_score"]
            assert compared[code]["score"] == item["score"]


async def test_compare_deduplicates_repeated_ids(
    client, auth_headers, profile_ready, seeded_catalog
):
    dev = await _job_id(client, auth_headers, "software-developer")
    nurse = await _job_id(client, auth_headers, "nurse")
    response = await client.get(
        f"/api/v1/match/compare?job_ids={dev},{nurse},{dev}", headers=auth_headers
    )
    assert response.status_code == 200
    rows = response.json()
...
async def test_compare_validates_input(
    ...
    one = await client.get(f"/api/v1/match/compare?job_ids={dev}", headers=auth_headers)
    assert one.status_code == 422

    five = await client.get(
        f"/api/v1/match/compare?job_ids={dev},{nurse},{firefighter},{physician},{ghost}",
        headers=auth_headers,
    )
    assert five.status_code == 422

    garbage = await client.get(
        f"/api/v1/match/compare?job_ids={dev},not-a-uuid", headers=auth_headers
    )
