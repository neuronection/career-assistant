...
export function UniversityDetail() {
  ...
  const deadlines: Deadline[] = useMemo(() => {
    ...
    return university.departments
      ...
      .map((d) => ({ date: d.application_deadline as string, label: `${t("universityDetail.apply")}: ${d.name}` }));
  }, [university]);

  if (error) return <p className="text-rose-600">{error}</p>;
  if (!university) return <p className="text-slate-400">{t("common.loading")}</p>;

  return (
    <div className="max-w-5xl mx-auto space-y-6" data-testid="university-detail">
      <div>
        <Link to="/catalog/universities" className="text-sm text-slate-400 hover:text-slate-600 inline-flex items-center gap-1">
        ...
        <div className="flex items-start justify-between mt-2">
          ...
          <Button size="sm" onClick={() => setAddOpen(true)}>
            <Plus className="w-4 h-4" /> {t("universityDetail.addDepartment")}
          </Button>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <section className="space-y-4">
          {university.departments.length === 0 ? (
            <EmptyState
              title={t("universityDetail.noDepartmentsTitle")}
              description={t("universityDetail.noDepartmentsBody")}
            />
          ) : (
            university.departments.map((d) => (
              <div key={d.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                ...
                <Table
                  headers={[t("jobDetail.year"), t("universityDetail.baseline"), t("universityDetail.top"), t("universityDetail.quota"), t("universityDetail.source")]}
                  emptyText={t("universityDetail.noBaselines")}
                  rows={d.admissions
                    .slice()
  ...
  return (
    <Modal open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <ModalContent size="md" aria-describedby={undefined}>
        <ModalHeader>
          <ModalTitle>{t("universityDetail.addDepartment")}</ModalTitle>
        </ModalHeader>
        <div className="space-y-4 px-6 pb-6">
          <label className="block text-sm">
            ...
            <input
              ...
              placeholder={t("universityDetail.deptNamePlaceholder")}
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              placeholder={t("universityDetail.fieldKeyPlaceholder")}
              className="mt-1 w-full border border-slate-200 rounded-lg px-3 py-2 text-sm"
            />
          </label>
          <div className="text-sm">
            ...
            <div className="mt-1">
              <DatePicker value={deadline} onChange={setDeadline} allowClear placeholder={t("universityDetail.pickDate")} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <label className="block text-sm">
              ...
              <input
                ...
                placeholder={t("universityDetail.baselinePlaceholder")}
                className="mt-1 w-full border border-slate-200 rounded-lg px-3 py-2 text-sm"
              />
            </label>
            <label className="block text-sm">
