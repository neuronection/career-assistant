"""Pluggable question-kind registry (Phase 23; registry v2).
...
(same fairness stance as), never zeroing.
...
eligibility gates and
{"evidence": "..."} for prose answers.

 registry v2: kinds are registered objects carrying
...
def _merged() -> dict:
    """Fresh delta accumulator: skill + dimension levels, interest keys."""
    return {"skill_levels": {}, "interest_keys": [], "dimension_levels": {}}
    ...
    def derive(self, question: dict, answer: dict) -> dict:
        ...
        if dimension_key:
            return {
                "skill_levels": {},
                "interest_keys": [],
                "dimension_levels": {dimension_key: float(answer["value"])},
            }
        if not skill_key:
...
class LikertMatrix:
    ...
    def derive(self, question: dict, answer: dict) -> dict:
        ...
        for statement_id, value in answer["values"].items():
            ...
            for key, delta in (scores.get("dimension_levels") or {}).items():
                merged["dimension_levels"][key] = (
                    merged["dimension_levels"].get(key, 0) + float(delta) * weight
                )
            for key in scores.get("interest_keys") or []:
                if key not in merged["interest_keys"]:
                    merged["interest_keys"].append(key)
...
class EligibilityGate:
    """Factual boolean/select (license, work auth) — feeds gates,
    never skill deltas."""
