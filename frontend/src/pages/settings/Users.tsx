...
export function Users() {
  ...
  const doReset = async () => {
    ...
    try {
      ...
      setNote(
        t("users.resetNote", { email: resetting.email })
      );
      setResetting(null);
      setNewPassword("");
      await load();
    ...
    <div className="space-y-4" data-testid="settings-users">
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-slate-500">
              <tr>
                ...
                <th className="px-4 py-2">{t("users.matches")}</th>
                <th className="px-4 py-2">{t("users.role")}</th>
                <th className="px-4 py-2">{t("users.status")}</th>
                ...
                return (
                  <tr key={u.id} className="border-t border-slate-100">
                    <td className="px-4 py-2">
                      {u.email}
                      {isSelf && <span className="text-xs text-slate-400"> ({t("users.you")})</span>}
                    </td>
                    <td className="px-4 py-2 text-slate-500">{u.insight_count}</td>
                    <td className="px-4 py-2">
                      {u.is_admin ? (
                        <span className="text-primary-700">{t("users.admin")}</span>
                      ) : (
                        t("users.user")
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2 text-right space-x-2 whitespace-nowrap">
                      {!isSelf && (
                        <>
                          ...
                          >
                            {u.is_admin ? t("users.demote") : t("users.promote")}
                          </button>
                          <button
      ...
      {resetting && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-2 max-w-md">
          <p className="text-sm font-medium">{t("users.newPasswordFor", { email: resetting.email })}</p>
          <input
            type="text"
            value={newPassword}
