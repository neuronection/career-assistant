"""Experience service: structured CRUD, derivation, evidence.
...
explicitly — conflicts with existing self-report levels route through the
 reconciliation shape (recorded, never silently overwritten).
"""
...
from app.models.enums import SkillOrigin, SkillStatus
...
class ExperienceService:
    ...
    async def _propose_skills(self, keys: list[str]) -> dict[str, Skill]:
        """Find-or-propose `proposed` skills for free-text keys (no
        duplicates); self-reporting never hard-fails — promotion to
        active stays an admin decision."""
        ...
        for key in keys:
            ...
            if existing is None:
                existing = Skill(
                    key=norm,
                    label=norm.replace("-", " ")[:120],
                    category="general",
                    status=SkillStatus.PROPOSED.value,
                    origin=SkillOrigin.USER.value,
                    provenance={"via": "experience"},
                )
    ...
    async def _replace_skills(self, item: ExperienceItem, skills: list[dict]) -> None:
        ...
        unknown = sorted({k for k in keys if k not in rows})
        if unknown:
            rows.update(await self._propose_skills(unknown))
        for entry in skills:
