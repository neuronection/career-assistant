...
async def market_snapshot(
    ...
    *,
    ...
    # Top hiring orgs aggregate by the normalized org entity:
    # label variants of one organization collapse onto org_id; postings
    # without a matched org fall back to their raw label.
    employers: dict[str, int] = {}
    org_names: dict[UUID, str] = {}
    org_ids = {p.org_id for p in postings if p.org_id is not None}
    if org_ids:
    ...
    for p in postings:
        if p.org_id is not None:
            key = f"org:{p.org_id}"
            employers[key] = employers.get(key, 0) + 1
        elif p.org:
            key = f"label:{p.org}"
            employers[key] = employers.get(key, 0) + 1
    ...
    return {
        ...
        "top_employers": [
            {
                "org": (
                    org_names.get(UUID(key.removeprefix("org:")))
                    if key.startswith("org:")
                    else key.removeprefix("label:")
                ),
                "count": count,
            }
            for key, count in sorted(employers.items(), key=lambda kv: -kv[1])[:5]
        ],
...
async def checkin_status(db: AsyncSession, user_id: UUID) -> dict:
    """Quarterly check-in state — owned by the scheduler.
