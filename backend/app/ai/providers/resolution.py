...
async def _find_tier_assignment(
    ...
    conditions = [
        ...
        AITaskAssignment.is_active.is_(True),
        AITaskAssignment.scope == scope,
        AITaskAssignment.tier.is_not(None),
    ]
    if scope == "user":
        if user_id is None:
            return None
        conditions.append(AITaskAssignment.user_id == user_id)
    rows = await db.execute(select(AITaskAssignment.tier).where(*conditions).limit(1))
...
async def _resolve_scanned(
    ...
    kind the scan order is identical (task first, user scope first).
    """
    ...
    for task in (task_type, DEFAULT_TASK):
        for scope in ("user", "system"):
            tier = await _find_tier_assignment(db, task, scope, user_id)
            if tier is None:
                continue
            found = await _find_tier_model(db, tier, user_id)
            if found:
                provider, model = found
                return ResolvedModel(
                    provider_type=provider.provider_type,
                    ...
                    api_key=provider.api_key,
                    model_name=model.model_name,
                    source=f"{scope}:{task} (tier:{tier}){source_suffix}",
                    temperature=model.temperature,
                    max_tokens=model.max_tokens,
                    reasoning_effort=model.reasoning_effort,
                    tier=tier,
...
def known_task_types() -> list[dict]:
    ...
    tasks = []
    for t in AITaskType:
        task_def = TASKS_BY_NAME[t.value]
        tasks.append(
            {
                "value": t.value,
