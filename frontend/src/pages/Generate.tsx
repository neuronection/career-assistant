...
export function Generate() {
  ...
  const onDone = async (finished: BackgroundJob) => {
    ...
    if (finished.status === "succeeded") {
      ...
      setError(finished.error ?? t("generate.failed"));
    } else {
      setError(t("generate.cancelled"));
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto" data-testid="generate">
      <p className="text-slate-500">{t("generate.subtitle")}</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-4">
        <div className="flex gap-2">
          {(["general", "prompt"] as const).map((m) => (
            ...
            >
              {m === "general" ? t("generate.exploreBroadly") : t("generate.fromPrompt")}
            </button>
          ))}
        </div>
        {mode === "prompt" && (
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-slate-500">{t("generate.howMany")}</label>
          <input
            type="number"
            min={1}
            max={15}
          ...
          >
            <Sparkles className="w-4 h-4" /> {busy ? t("generate.generating") : t("generate.generate")}
          </button>
        </div>
        {busy && job && (
          <JobFlowStatus job={job} title={t("generate.generatingDrafts")} testId="job-progress" />
        )}
        ...
        <section className="space-y-3" data-testid="review-queue">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">
              {t("generate.reviewQueue", { count: queue.length })}
            </h2>
            <div className="flex gap-2">
              <button
      ...
      {drafts.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">{t("generate.draftsCreated", { count: drafts.length })}</h2>
            <button
              onClick={async () => {
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
            {drafts.map((d) => (
              <div key={d.id} className="space-y-1">
                <JobCard job={d} />
                <Link to={`/jobs/${d.code}`} className="text-xs text-primary-700">{t("generate.openDetails")} →</Link>
              </div>
            ))}
          </div>
        </section>
