...
class MatchingService:
    ...
    async def compare(self, profile: Profile, *, job_ids: list[UUID]) -> list[dict]:
        """Side-by-side comparison rows, in request order.

        Same deterministic layer as `rankings` — blended score, fresh
        (backfilled) insights, gate flags. No AI, no new tables.
        """
        ...
        rows = await self.db.execute(
            ...
            .where(Job.id.in_(job_ids), Job.status == "published")
        )
        jobs = {job.id: job for job in rows.scalars().unique().all()}
        if len(jobs) != len(job_ids):
            raise NotFoundError("Job not found")
        insights = await self.backfilled_insights(profile)
        items = []
        for job_id in job_ids:
            insight = insights.get(job_id)
            if insight is None or insight.fit_score is None:
                raise NotFoundError("Job not found")
            fit_score = float(insight.fit_score)
