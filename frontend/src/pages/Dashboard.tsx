...
const STAGE_TAGLINE: Record<string, string> = {
  student: "dashboard.tagline.student",
  early_career: "dashboard.tagline.earlyCareer",
  experienced: "dashboard.tagline.experienced",
  switching: "dashboard.tagline.switching",
  returning: "dashboard.tagline.returning",
};
...
export function Dashboard() {
  ...
  const requiredLeft = completeness
    ? Object.entries(completeness.required ?? {}).filter(
        ([key, req]) => req && !completeness.sections[key]
      ).length
    : 0;
    ...
    <div className="space-y-8" data-testid="dashboard">
      <section>
        ...
        <p className="text-slate-500 mt-1" data-testid="dashboard-tagline">
          {bootstrap
            ? t(STAGE_TAGLINE[bootstrap.career_stage] ?? STAGE_TAGLINE.student)
            : t("dashboard.taglineFallback", { count: jobCount })}
        </p>
      </section>

      {completeness && requiredLeft > 0 && (
        <section className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between" data-testid="completeness-banner">
          <div>
            <p className="font-medium text-amber-900">
              {t("dashboard.sectionsLeft", { count: requiredLeft })}
            </p>
            <p className="text-sm text-amber-700">
              {t("dashboard.completenessHint", { percent: completeness.percent })}
            </p>
          </div>
          <Link
            to="/onboarding"
      ...
      <section className="grid md:grid-cols-2 xl:grid-cols-4 gap-4">
        <QuickLink to="/catalog/generate" icon={Sparkles} title={t("dashboard.quickGenerateTitle")} text={t("dashboard.quickGenerateText")} />
        <QuickLink to="/cv?generate=1" icon={FileText} title={t("dashboard.quickCvTitle")} text={t("dashboard.quickCvText")} />
        <QuickLink to="/rankings" icon={Trophy} title={t("dashboard.quickRankingsTitle")} text={t("dashboard.quickRankingsText")} />
        {(!bootstrap || bootstrap.features.universities) && (
          <QuickLink to="/catalog/universities" icon={Building2} title={t("dashboard.quickUniversitiesTitle")} text={t("dashboard.quickUniversitiesText")} />
        )}
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <h2 className="font-semibold text-slate-900">{t("dashboard.topMatches")}</h2>
            <FitStaleBadge />
          </div>
          <Link to="/rankings" className="text-sm text-primary-700">{t("dashboard.seeAll")}</Link>
        </div>
        {top.length === 0 ? (
          <div className="bg-white border border-dashed border-slate-300 rounded-xl p-6 text-center text-sm text-slate-500">
            <p>{t("dashboard.noScoresYet")}</p>
            <Link to="/rankings" className="text-primary-700 font-medium">{t("dashboard.scoreFirstJobs")}</Link>
          </div>
        ) : (
      ...
      <section data-testid="feed-section">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-slate-900 flex items-center gap-2">
            {feedView === "saved" ? t("dashboard.savedJobs") : t("dashboard.newForYou")}
            {feedView === "all" && feed && feed.unseen > 0 && (
              ...
              >
                {t("dashboard.unseenCount", { count: feed.unseen })}
              </span>
            )}
          </h2>
          <div className="flex items-center gap-3">
        ...
        {feed && feed.items.length > 0 ? (
          ...
          <div className="bg-white border border-dashed border-slate-300 rounded-xl p-6 text-center text-sm text-slate-500">
            {feedView === "saved" ? (
              <p>{t("dashboard.nothingSaved")}</p>
            ) : (
              <p>{t("dashboard.feedCaughtUp")}</p>
            )}
          </div>
        )}
      </section>
      ...
      {target && target.families.length > 0 && (
        <section className="bg-primary-50 border border-primary-100 rounded-xl p-4" data-testid="target-mode">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-slate-900 flex items-center gap-2">
              <Target className="w-4 h-4 text-primary-600" /> {t("dashboard.targetMode")}
              <span className="text-xs font-normal text-slate-500">
                {target.families.map((f) => f.replace(/-/g, " ")).join(" · ")}
              </span>
            </h2>
            <Link to="/postings" className="text-sm text-primary-700">{t("dashboard.liveTab")}</Link>
          </div>
          ...
          {target.nudges.length > 0 && (
            <div className="mt-3 bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between gap-2" data-testid="nudge-banner">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Link to="/profile/assessment" className="text-xs bg-primary-600 text-white rounded-lg px-3 py-1.5">
                  {t("dashboard.microRun")}
                </Link>
                <button
                  aria-label={t("dashboard.dismissNudge")}
                  onClick={() => {
          ...
          {targetJobs.length > 0 && (
            <div className="mt-3">
              <p className="text-xs font-medium text-slate-400 uppercase mb-2">{t("dashboard.openTargetJobs")}</p>
