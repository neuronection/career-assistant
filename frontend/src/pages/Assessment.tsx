      ...
      <div className="space-y-6 max-w-3xl mx-auto" data-testid="assessment-results">
        ...
        {effects && (effects.skill_conflicts as unknown[])?.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm">
            <p className="font-medium text-amber-800 mb-1">{t("assessment.conflictsTitle")}</p>
            <ul className="space-y-1">
              {(effects.skill_conflicts as { key: string; self_level: number; assessed_level: number }[]).map((c) => (
                <li key={c.key}>
                  {t("assessment.conflictLine", { self: c.self_level, key: c.key, assessed: c.assessed_level })}
          ...
          <ol className="text-sm space-y-1">
            {results.shortlist.slice(0, 5).map((s, i) => (
              <li key={s.job_id} className="flex items-center gap-2">
                ...
                {t("assessment.fit")} <strong>{Number(s.fit_score).toFixed(1)}</strong>
                <Link to="/rankings" className="text-primary-700 text-xs">{t("assessment.seeRankings")}</Link>
              </li>
            ))}
          </ol>
        </section>
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto" data-testid="assessment">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          {busy ? t("assessment.working") : state.phase_one_form ? t("assessment.startScenarios") : t("assessment.saveContinue")}
        </button>
      </div>
    </div>
  );
