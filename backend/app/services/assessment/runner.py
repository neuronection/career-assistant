...
from app.models.enums import AssessmentKind, AssessmentStatus, UserMetricSource
...
class AssessmentService:
    ...
    async def advance(self, run: AssessmentRun, user_id: UUID) -> dict:
        ...
        # Phase outputs are run-scoped state (: context stores state;
        # templates read the same slot).
    ...
    async def _derived_map(self, run: AssessmentRun) -> dict:
        merged: dict = {"skill_levels": {}, "interest_keys": [], "dimension_levels": {}}
    ...
    async def _apply_results(self, run: AssessmentRun, derived: dict) -> dict:
        ...
        template runs normalize their accumulated raw deltas
        through the template's own block first; every applied skill gains
        a `skill_evidence` row linking the run."""
        ...
        if run.kind == AssessmentKind.TEMPLATE.value and run.template_id:
            ...
            derived = {
                **derived,
                ...
                "dimension_levels": compiled.get("dimension_levels") or {},
        ...
        if interest_keys:
            ...
            await MetricService(self.db).recompute_interest_affinity(run.user_id)

        dimension_levels = derived.get("dimension_levels") or {}
        ...
        if dimension_levels:
            ...
            for key, value in sorted(dimension_levels.items()):
                try:
                    await service.upsert_metric(
                        run.user_id,
                        key,
                        float(value),
                        source=UserMetricSource.ASSESSMENT,
                        confidence=0.8,
                        evidence={
                            "basis": "assessment.run",
                            "run_id": str(run.id),
                        },
                    )
                    applied_dimensions += 1
                except ValidationError:
                    skipped_dimensions.append(key)
