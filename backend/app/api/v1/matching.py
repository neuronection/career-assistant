...
def _ranked_item(item: dict) -> RankedJob:
    """Serialise a service ranking/compare row (same shape for both)."""
...
async def rankings(
    ...
    return RankingsOut(
        total=result["total"],
        items=[_ranked_item(item) for item in result["items"]],
    )


@router.get("/match/compare", response_model=list[RankedJob])
async def compare_jobs(
    job_ids: str = Query(..., description="comma-separated job ids (2-4)"),
    user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> list[RankedJob]:
    """Side-by-side comparison for 2-4 catalog jobs, in request order."""
    from uuid import UUID as UUIDType

    ids: list[UUIDType] = []
    for part in job_ids.split(","):
        part = part.strip()
        if not part:
            continue
        try:
    ...
    try:
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, str(exc)) from exc
    return [_ranked_item(item) for item in items]
