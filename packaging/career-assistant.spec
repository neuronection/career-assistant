...
#
# Env overrides:
#   CA_ONEFILE=1|0  single-file exe (default: 1 on Windows/macOS, 0 on Linux)
#   CA_CONSOLE=1    keep a console attached (debugging)
import glob
import os
import platform
import re
...
ONEFILE = os.environ.get("CA_ONEFILE", "1" if (IS_WINDOWS or IS_MACOS) else "0") == "1"
CONSOLE = os.environ.get("CA_CONSOLE", "0") == "1"
...
ENTRY = os.path.join(ROOT, "backend", "careerassistant", "__main__.py")
...
if IS_LINUX:
    typelib_dirs = [
        d for d in os.environ.get("GI_TYPELIB_PATH", "").split(":") if d and os.path.isdir(d)
    ]
    for pattern in (
...
exe_kwargs = dict(
    name="CareerAssistant" if (IS_WINDOWS or IS_MACOS) else "careerassistant",
    debug=False,
...
if ONEFILE:
    ...
    coll = COLLECT(exe, a.binaries, a.datas, strip=False, upx=False, name="careerassistant")
