"""Assessment template library: tests are content, not code.

A template is a pydantic-validated package that compiles onto the
 engine — phases materialize as ordinary run questions, scoring
rides the existing kind handlers, and results normalize through the
template's own block. Versions are immutable rows: an edit
...
class AssessmentTemplate(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    contract (NULL-based uniques are unreliable across
    dialects). `ref` is the short in-instance share code;
    `content_hash` is the canonical-JSON sha256 of `content`
    verified on export/import.
    """
