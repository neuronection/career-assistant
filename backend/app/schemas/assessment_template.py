"""Assessment template content schemas.

Templates are content, not code: a pydantic-validated package that
compiles onto the engine. Every referenced `skill_key` must
resolve to a taxonomy id at save/import time;
...
class OptionScores(BaseModel):
    ...
    # Registry dimension deltas: keys resolve against
...
class TemplateQuestion(BaseModel):
    ...
    def _validate_scores(items) -> None:
        for item in items:
            ...
            for key, value in (scores.dimension_levels or {}).items():
