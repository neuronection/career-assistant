...
's three-concern split: `notifications` holds immutable *events*,
...
class NotificationKind(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """The kind registry every emitter resolves through.

    Seeded rows are system data; features add kinds with their phase. The
    `key` is the stable string space shared with's registry.
    """
    ...
    # Channel slots are's registry; `in_app` is the only always-on one.
...
class NotificationSubscription(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """A browser push subscription (web mode; VAPID).
...
class NotificationPreference(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Per-user channel preferences (Phase 30; extends per-kind).
