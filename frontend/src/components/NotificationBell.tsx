    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-96 p-0">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <span className="font-medium text-sm text-slate-900">{t("notificationBell.title")}</span>
          <button
            onClick={markAll}
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              ...
              >
                ...
                <button
                  type="button"
                  aria-label={t("notificationBell.dismissAria")}
        ...
        {desktopChannel && prefs && (
          ...
          >
            ...
            <label className="flex items-center justify-between text-xs text-slate-600 mt-2">
              <span>{t("notificationBell.quietFrom")}</span>
              <input
                type="time"
            ...
            <label className="flex items-center justify-between text-xs text-slate-600 mt-2">
              <span>{t("notificationBell.quietUntil")}</span>
              <input
                type="time"
        ...
        {fitRule && (
          <div className="px-4 py-3 border-t border-slate-100 bg-slate-50 rounded-b-lg">
            <label className="flex items-center justify-between text-xs text-slate-600">
              <span>{t("notificationBell.fitAlerts")}</span>
              <input
                type="checkbox"
            ...
            <label className="flex items-center justify-between text-xs text-slate-600 mt-2">
              <span>{t("notificationBell.notifyFromFit", { score: Number(fitRule.params.min_fit).toFixed(0) })}</span>
              <input
                type="range"
                min={0}
                max={10}
