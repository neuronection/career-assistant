...
async def test_quiet_hours_suppress_toast_but_inbox_filled(db, kinds):
    """Dispatch guard (: quiet hours kill the toast, never
    the inbox row)."""
    from app.desktop.notifier import DesktopChannel
    from app.services import notification_channels
    from app.services.notification_service import NotificationService
...
def _tray_actions(conn):
    from app.desktop.tray import TrayActions

    # The tray simulates a second session, but under the rollback-based
    # clean_db it must share the test's transaction to see its data.
...
async def _first_admin_id(conn):
    from app.models.user_model import User

    async with _test_session(conn) as db:
        rows = await db.execute(select(User.id).where(User.is_admin.is_(True)))
        return rows.scalars().first()


async def test_tray_actions_use_admin_inbox(db, clean_db, client, auth_headers, kinds):
    ...
    admin_id = await _first_admin_id(clean_db)
    await NotificationService(db).emit("fit_threshold", [admin_id], title="yours")
    await db.commit()

    actions = _tray_actions(clean_db)
...
async def test_tray_status_counts(db, clean_db, client, auth_headers, kinds):
    from app.services.notification_service import NotificationService
    ...
    status = _tray_actions(clean_db).tray_status()
    status = await status
