...
class SchedulerService:
    ...
    async def ensure_system_schedules(self) -> None:
        ...
        defaults = [
            ...
            (
                ScheduleKind.SYSTEM_CATALOG_ENRICH,
                BackgroundJobType.CATALOG_ENRICH.value,
                {"type": "boot_stale", "params": {"older_than_minutes": 60 * 24 * 7}},
                {"limit": 5},
                MisfirePolicy.SKIP.value,
            ),
            (
                ScheduleKind.SYSTEM_FOLLOWUPS,
                BackgroundJobType.FOLLOWUP_SWEEP.value,
                {
                    "type": "interval",
            ...
            (
                ScheduleKind.SYSTEM_MARKET_HISTORY,
                BackgroundJobType.MARKET_HISTORY_CAPTURE.value,
                {
                    "type": "interval",
                    "params": {"every_minutes": 60 * 24, "jitter_minutes": 60},
    ...
    async def set_autopilot_schedule(
        ...
        rows = await self.db.execute(
            select(AutopilotGoal).where(
                AutopilotGoal.id == goal_id, AutopilotGoal.user_id == user_id
            )
        )
        goal = rows.scalars().first()
        if goal is None:
            raise ValidationError("Autopilot goal not found")
        existing = await self.db.execute(
            select(Schedule).where(
        ...
        if goal.status != "active":
            raise ValidationError("Paused goals cannot be scheduled")
        trigger_registry.resolve_trigger(trigger)
        goal.cadence = trigger
        schedule = await self.ensure_schedule(
            kind=ScheduleKind.USER_AUTOPILOT.value,
            owner_user_id=user_id,
            task=BackgroundJobType.AUTOPILOT_RUN.value,
            trigger=trigger,
    ...
    async def tick(self, now: Optional[datetime] = None) -> int:
        ...
        for schedule_id in due_ids:
            # Claim marker (single-process assumption, same as): a
