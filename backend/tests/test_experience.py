"""Experience profile: derivation math, CRUD, apply, evidence."""
...
async def test_unknown_skill_key_auto_proposed(
    ...
    assert proposed.origin == "user"
    assert proposed.provenance == {"via": "experience"}

    second = await client.post(
        "/api/v1/me/experience",
        json={
            "title": "Another rust thing",
            "kind": "project",
            "start": "2025-07-01",
            "end": "2025-08-31",
            "skills": [{"skill_key": "rust-embedded"}],
        },
        headers=auth_headers,
    )
