...
class AITaskAssignment(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    A row binds either a concrete model (``model_id`` set — explicit
    override) or a tier (``tier`` set, ``model_id`` null — the task
    routes to the best active model registered for that tier).
    """
