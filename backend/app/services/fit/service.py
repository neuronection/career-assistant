...
class FitService:
    ...
    async def user_context(self, profile: Profile) -> dict:
        ...
        riasec = await self._riasec_vector(profile.user_id)
        return {
            "skill_levels": {skill_id: int(level) for skill_id, level in skill_rows},
            "education_level": await effective_education_level(
                self.db, profile.user_id, basics.get("education_level")
            ),
            ...
            "salary_min": _number_or_none(constraints.get("salary_min")),
            "salary_negotiable": bool(constraints.get("salary_negotiable", False)),
            "interest_ids": {str(i) for i in interest_ids},
            "work_style": {k: work_prefs.get(k, 3) for k in WORK_STYLE_SLIDERS},
            "riasec": riasec,
            "values": await self._values_vector(profile.user_id),
        }

    async def _values_vector(self, user_id: UUID) -> dict[str, float]:
        """Full dimension keys ("values.autonomy") — matches the job-side
        signal keys from `job_values_signal`."""
        rows = await self.db.execute(
            select(UserMetricProfile.dimension_key, UserMetricProfile.value).where(
                UserMetricProfile.user_id == user_id,
                UserMetricProfile.dimension_key.like("values.%"),
            )
        )
        return {key: float(value) for key, value in rows.all()}

    async def _riasec_vector(self, user_id: UUID) -> dict[str, float]:
        """Stored interest-affinity metric rows → {letter: value}."""
        ...
        for key, value in rows.all():
            letter = key.removeprefix("interest.")
            if letter in RIASEC_LETTERS:
                vector[letter] = float(value)
        return vector
    ...
    async def job_context(self, job) -> dict:
        ...
        riasec_letters = await MetricService(self.db).job_riasec_letters(job)
        return {
            "skill_links": [
                {
        ...
        if stored:
            ...
            stage, _source = effective_stage(
                ...
                education_level=await effective_education_level(
                    self.db,
                    profile.user_id,
                    (profile.basics or {}).get("education_level"),
                ),
            )
