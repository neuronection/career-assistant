...
export async function selectChatMessage(messageId: string): Promise<ChatMessage[]> {
  const { data } = await api.post<ChatMessage[]>(`/chat/messages/${messageId}/select`);
  return data;
}
...
export async function fetchChatTree(sessionId: string): Promise<ChatTree> {
  const { data } = await api.get<ChatTree>(`/chat/sessions/${sessionId}/tree`);
  return data;
}
