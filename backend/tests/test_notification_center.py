"""Notification center: fan-out, dispatch log, threads, prefs,
...
async def test_browser_push_dead_endpoint_cleanup(db, clean_db, kinds, monkeypatch):
    ...
    # The channel opens its own session (AsyncSessionLocal); under the
    # rollback-based clean_db it must share the test's transaction to see
    # and update the test's subscription.
    ...
    monkeypatch.setattr(webpush_service, "AsyncSessionLocal", factory)
